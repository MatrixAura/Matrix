...
import me.matrixaura.matrix.core.config.ConfigContainer;
import me.matrixaura.matrix.gui.Panel;
import me.matrixaura.matrix.gui.renderers.ClickGUIRenderer;
...
import me.matrixaura.matrix.utils.ListUtil;
...
public class ConfigManager {
    ...
    public static final String CONFIG_PATH = "Matrix/config/";
    private static final Gson gsonPretty = new GsonBuilder().setPrettyPrinting().create();
    private static final JsonParser jsonParser = new JsonParser();

    private final File CLIENT_FILE = new File(CONFIG_PATH + "Matrix_Client.json");
    private final File GUI_FILE = new File(CONFIG_PATH + "Matrix_GUI.json");
    ...
    public void saveGUI() {
        try {
            ...
            Matrix.log.error("Error while saving GUI config!");
            e.printStackTrace();
        }
    }
    ...
    public void loadGUI() {
        if (GUI_FILE.exists()) {
            try {
                ...
                Matrix.log.error("Error while loading GUI config!");
                e.printStackTrace();
            }
        }
    }
    ...
    public void saveClient() {
        try {
            ...
            Matrix.log.error("Error while saving client stuff!");
            e.printStackTrace();
        }
    }
    ...
    private void loadClient() {
        if (CLIENT_FILE.exists()) {
            try {
                ...
                Matrix.log.error("Error while loading client stuff!");
                e.printStackTrace();
            }
        }
    }
    ...
    private void saveClientStuff(JsonObject father) {
        ...
        stuff.addProperty("ChatSuffix", Matrix.CHAT_SUFFIX);
    ...
    private void trySetClient(JsonObject json) {
        try {
            ...
            Matrix.CHAT_SUFFIX = json.get("ChatSuffix").getAsString();
            FakePlayer.customName = json.get("FakePlayerName").getAsString();
        } catch (Exception e) {
            Matrix.log.error("Error while setting client!");
        }
    }
